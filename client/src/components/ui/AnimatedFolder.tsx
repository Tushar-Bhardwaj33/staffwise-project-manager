...
// Simple helper to get an icon based on extension
...
export const AnimatedFolder: React.FC<AnimatedFolderProps> = ({ documents, onSearch }) => {
  ...
  // We only show up to 5 files in the animation. 
  // We reverse them so file-5 renders first in DOM, then file-4, etc. 
  // This matches the original CSS stacking context.
        ...
        <div className="folder-container">
          ...
          <div className="folder-search">
            ...
            <input 
              ...
              onClick={(e) => e.stopPropagation()} // prevent closing folder when clicking input
            />
          </div>

          {/* Render files dynamically */}
          {previewDocs.map((doc, index) => {
            // Original code expected class file-5 down to file-1
            // We have index 0..4. If length is 5, index 0 -> file-5.
            const fileNum = previewDocs.length - index; 
            const ext = doc.filename.split('.').pop() || '';
            return (
              <div key={doc._id} className={`file file-${fileNum}`}>
                <div className="shine" />
                {getFileIcon(ext)}
                <div className="file-text truncate px-3 py-2 pr-8">{doc.filename}</div>
                <div className="file-tag">{ext.toUpperCase()} • {formatBytes(doc.size)}</div>
              </div>
            );
          })}
          ...
          <div className="folder-front-wrapper">
            ...
            <div className="counter">
              ...
              <span className="counter-number">{documents.length.toString().padStart(2, '0')}</span>
            </div>
          </div>
        </div>
      </label>
